...
% OUTPUTS
% __gape_array:       A cell array of equal size as emg. Each element of the
%                     cell array is a matrix with dimensions trial x time
% HELPER FUNCTIONS
% __burst_features(): Calculates the features for individual movements

%Written by Jennifer Li

function gape_array=get_gapes(emg,onset_or_peak)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for tastes=1:length(features) %% loop through each taste
        ...
        gape_array{tastes}=zeros(no_of_trials,trial_length); %%% preallocating 
